...
from watcher_core import CloudSync, Watcher, _strip_version_from_name, log
...
class DashboardMixin:
    """Mixin for MainWindow that provides the Dashboard tab and all related helpers."""
    ...
    def _on_notif_clicked(self, action_tab: str, notif_type: str, notif_data: dict):
        ...
        if notif_type == "achievement_beaten":
            try:
                import Achievement_watcher as _aw_mod
                dlg = _aw_mod.AchievementBeatenDialog(self.cfg, notif_data, parent=self)
                dlg.exec()
            except Exception:
                pass
            return
