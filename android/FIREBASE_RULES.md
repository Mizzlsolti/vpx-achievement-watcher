...
2. **Existing Rules**: The rules above should be **merged** with the existing rules for `players/`, `duels/`, `tournaments/`, and `tournament_chat/` nodes. Do not replace existing rules.
...
## Existing Chat Rules Reference

The tournament chat already has these rules documented in `ui/chat.py`:
