...
class MainWindow(QMainWindow, HotkeysMixin, OverlayCtrlMixin, TrayMixin, CloudStatsMixin, AWEditorMixin, SystemMixin, AppearanceMixin, OverlaysMixin, ProgressMixin,
    ...
    _TAB_HELP = {
        ...
        "appearance_overlay": (
            ...
            "(Main Overlay, Toast, System Notifications, Status Overlay, "
            "Duel Notifications).<br>"
            ...
            "the overlay.<br>"
            "• <b>📺 Duel Picture-in-Picture Stream</b>: During a duel, your playfield is "
            "streamed to your opponent and their playfield appears in a small PiP window on your "
            "screen. Use <b>FPS</b> and <b>Quality</b> to balance stream smoothness against CPU "
            "load. Click <b>Place</b> to drag and resize the PiP window to your preferred position "
            "(click again to save). Click <b>Test</b> to preview the PiP window with a placeholder "
            "— no active duel required.<br>"
    ...
    def _build_tab_controls(self):
        ...
        self.cmb_ch_left_src = QComboBox(); self.cmb_ch_left_src.addItems(["keyboard", "joystick"]); self.cmb_ch_left_src.setCurrentText(self.cfg.OVERLAY.get("duel_left_input_source", "keyboard")); self.cmb_ch_left_src.currentTextChanged.connect(lambda s: self._on_nav_src_changed("left", s))
        ...
        self.cmb_ch_right_src = QComboBox(); self.cmb_ch_right_src.addItems(["keyboard", "joystick"]); self.cmb_ch_right_src.setCurrentText(self.cfg.OVERLAY.get("duel_right_input_source", "keyboard")); self.cmb_ch_right_src.currentTextChanged.connect(lambda s: self._on_nav_src_changed("right", s))
