...
class MainWindow(QMainWindow, CloudStatsMixin):
    ...
    _TAB_HELP = {
        ...
        "dashboard": (
            ...
            "• <b>Session Summary</b>: Two cards side by side:<br>"
            "&nbsp;&nbsp;– <b>Last Run</b>: Table, score, achievements, and timestamp of the last completed session.<br>"
            "&nbsp;&nbsp;– <b>Run Status</b>: Live status of the current session — table, session state, "
            "cloud upload result, and leaderboard rank. "
            "Status dots indicate: "
            "<span style='color:#00C853;'>●</span> green = online/verified, "
            "<span style='color:#FFA500;'>●</span> yellow = pending/local, "
            "<span style='color:#FF3B30;'>●</span> red = cloud off or table not mapped.<br>"
            ...
            "• <b>Quick Actions</b>: <b>Restart Engine</b> restarts the watcher, "
            "<b>Minimize to Tray</b> hides the window to the system tray, "
            "<b>Quit GUI</b> closes the application."
        ),
        "progress": (
            "<b>📈 Progress</b><br><br>"
        ...
        "appearance": (
            ...
            "• <b>Global Styling</b>: Choose the font family, base size (8–64 pt), and overlay scale (30–300%). "
            "Click <b>100%</b> to reset the scale to default.<br>"
            "• <b>Widget Placement &amp; Orientation</b>: Position and rotate each overlay window. "
            "Use <b>Place</b> to drag the overlay to its position, and <b>Test</b> to preview it with sample data. "
            "Each overlay has <b>Portrait Mode (90°)</b> and <b>Rotate CCW</b> checkboxes.<br>"
            "• Overlays: Main Stats Overlay (with optional <b>Auto-Close 1 min</b>), Achievement Toast, "
            "Challenge Menu, Timers &amp; Counters, System Notifications, Heat Bar, "
            "and Status Overlay (with its own <b>Enabled</b> toggle).<br>"
            ...
            "• Pick one of the 15 colour themes from the dropdown (Neon Blue, Retro Arcade, Synthwave, etc.) "
            ...
            "<i>System Notification</i> and <i>Status Overlay</i> use fixed styles and are not affected by themes.<br>"
        ...
        "available_maps": (
            ...
            "• <b>🔄 Load List</b>: Scan your tables folder and reload the full list.<br>"
            ...
            "&nbsp;&nbsp;– <b>ROM</b>: The ROM (NVRAM) file name used by VPinMAME.<br>"
            "&nbsp;&nbsp;– <b>NVRAM Map</b>: ✅ = an NVRAM map file exists for this ROM (required to track scores &amp; achievements); ❌ = no map available.<br>"
            "&nbsp;&nbsp;– <b>Local</b>: 🟠 = a matching .vpx table file was found in your tables folder.<br>"
            "&nbsp;&nbsp;– <b>VPS-ID</b>: The assigned Virtual Pinball Spreadsheet ID. Required for cloud leaderboard uploads.<br>"
            "&nbsp;&nbsp;– <b>Author</b>: Table author(s) from the VPS database.<br>"
            "&nbsp;&nbsp;– <b>+</b>: Opens the detail dialog with VPS info and achievement data for that table.<br>"
            "• To appear on the cloud leaderboard a table needs <b>both</b> an NVRAM map (✅) "
            "<b>and</b> a VPS-ID assigned. Tables missing either will be tracked locally only."
        ),
        "controls": (
            ...
            try:
                ...
                with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as zf:
                    ...
                    if os.path.isfile(src):
                        ...
                        exported.append("config.json")

                    # VPS ID mapping
                    vps_src = f_vps_mapping(self.cfg)
                    if os.path.isfile(vps_src):
                        zf.write(vps_src, "vps_id_mapping.json")
                        exported.append("vps_id_mapping.json")

                    # Achievement state (unlocks, badges, level, etc.)
                    ach_src = f_achievements_state(self.cfg)
                    if os.path.isfile(ach_src):
                        zf.write(ach_src, "achievements_state.json")
                        exported.append("achievements_state.json")

                    # Challenge scores
                    ch_src = os.path.join(self.cfg.BASE, "challenge_scores.json")
            ...
            try:
                with zipfile.ZipFile(path, "r") as zf:
                    ...
                    if "vps_id_mapping.json" in names:
                        vps_dst = f_vps_mapping(self.cfg)
                        try:
                            ensure_dir(os.path.dirname(vps_dst))
                            with open(vps_dst, "wb") as fh:
                    ...
                    if "achievements_state.json" in names:
                        ach_dst = f_achievements_state(self.cfg)
                        try:
                            ensure_dir(os.path.dirname(ach_dst))
                            with open(ach_dst, "wb") as fh:
                    ...
                    if "challenge_scores.json" in names:
                        ch_dst = os.path.join(self.cfg.BASE, "challenge_scores.json")
                        try:
                            with open(ch_dst, "wb") as fh:
